...
Results(
  Dataset = "Test Data/Inputs/Dataset.xlsx",
  Template = "Test Data/Inputs/Template - Crosstabs.docx",
  Group1 = c("Participants", "Pre", "Unweighted"),
  Group2 = c("Participants", "Post", "Unweighted"),
  Outputs = "Test Data/Outputs")

```

```{r running}

setwd("/Users/willoughbywinograd/Library/CloudStorage/GoogleDrive-willoughby@winograd.org/My Drive/Employment/Current/Deliberations.US/Advising/Results/The Electoral College")

rm(list=ls()) # Clears global environment

library(DeliberativePolling) # Attaches custom package

Dataset = "/Inputs/Dataset.xlsx"
Template = "/Inputs/Template.docx"
Group1 = c("Participants", "Pre", "Unweighted")
Group2 = c("Participants", "Post", "Unweighted")
Outputs = "/Outputs"
Select_Demographic = "Political Party Preference"
...
Export_Word = function(Export, Outputs, Template, Crosstabs, Codebook, File_Name, Document_Title, Type, Demographic_Category){
  ...
  if(Type == "Responses"){
    ...
    # Sets font type
    Legend = font(Legend, fontname = "Times New Roman", part = "all")
    Titles = font(Titles, fontname = "Times New Roman", part = "all")
    SampleSizes = font(SampleSizes, fontname = "Times New Roman", part = "all")
    Crosstabs = font(Crosstabs, fontname = "Times New Roman", part = "all")
...

{
  ...
  repeat{
    ...
    Demographics_Group1 = as.matrix(Dataset_Group1[ColumnNumber_Demographics_Group1])
    Demographic_Category = colnames(Demographics_Group1)
    if(Select_Demographic == Demographic_Category){
    ...
    # Runs a pre-defined function that creates Excel spreadsheets from the crosstabs.
    {
      ...
      DemographicsCounter = DemographicsCounter_End
...
Changer = TRUE
for(Proposal in unique(Crosstabs$ID)[10:34]){
  print(Proposal)
  
  if(Changer){Type = "proposal"}
  if(Proposal == "Motivation 1"){
     Type = "statement"
     Changer = FALSE
  }
  ...
  Text = (Crosstabs[which(Crosstabs == Proposal), 2])
  
  Lines = paste0("For the ", Type, " \"", Proposal, "\", participants were asked to give their agreement that, \"", gsub("\\.", "", Text), "\".")
  
  # Create the matrix with the specified headings
  Data <- matrix(ncol=3, nrow=0, dimnames=list(NULL, c("Group", "Support Before Deliberation", "Support After Deliberation")))
  
  for(Group in Crosstabs[2:6, 2][-4]){
    
    Group_OG = Group
    
    Index = which(Group == Crosstabs[2:6, 2])
    if(Group == "Total"){
  ...
  Percentages = (Crosstabs[(which(Crosstabs == Proposal)+1):(which(Crosstabs == Proposal)+3), 14+Index])
  Change_Support = Convert_Percentage(Percentages[3])
  Change_Opposition = Convert_Percentage(Percentages[1])
  
  Percentages = (Crosstabs[(which(Crosstabs == Proposal)+1):(which(Crosstabs == Proposal)+3), 2+Index])
  Beg_Support = Convert_Percentage(Percentages[3])
  Beg_Opposition = Convert_Percentage(Percentages[1])
  
  Percentages = (Crosstabs[(which(Crosstabs == Proposal)+1):(which(Crosstabs == Proposal)+3), 8+Index])
  End_Support = Convert_Percentage(Percentages[3])
  End_Opposition = Convert_Percentage(Percentages[1])
  
  Data = rbind(Data, c(Group_OG, Beg_Support, End_Support))
  ...
  if(Stance(End_Support, End_Opposition) == Stance(Beg_Support, Beg_Opposition)){
    Line2 = paste("After deliberation there was", Stance(End_Support, End_Opposition), Note, "just as before deliberation.")
  } else {
    Line2 = paste("Before deliberation there was", Stance(Beg_Support, Beg_Opposition), "while after deliberation there was", Stance(End_Support, End_Opposition))
  }
  
  Lines = paste(Lines, paste0(Line1, ". ", Line2))
  }
  
  Data[Data == "Prefer not to state"] = "Other"
  Data[Data == "Total"] = "All"
  Data[Data == "Independent (no party affiliation)"] = "Indepedent"
  ...
  quartzFonts(myFont = c("Georgia", "plain", "normal", "normal"))
par(family="Georgia", cex=.6)
  Data[Data == 0] = .005
