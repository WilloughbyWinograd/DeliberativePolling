...
Crosstab.Ordinal = function(Dataset, Template, Group1, Group2, Outputs, Export, Alpha, Only_Significant){
  ...
  Crosstab = function(Dataset, Template, Output, Export){
    ...
    if(missing(Export)){Export = TRUE}
    if(missing(Outputs)){Outputs = ""}
    >>>>>>> d48c4bb2bab115ed8f4e35058c2cd7830cebfd39
    ...
    # Creates the name of the file.
    {
      ...
      File_Name = paste("Ordinal", Name_Group, Weights, sep = " - ")
  ...
  Crosstab.Nominal = function(Dataset, Template, Group1, Group2, Outputs, Export, Alpha, Only_Significant, Only_Means){
      ...
      Crosstab.Opinions = function(Dataset, Template, Group1, Group2, Outputs, Export, Alpha, Only_Significant, Only_Means){
        ...
        repeat{
          ...
          # Creates the name and title of the files.
          {
            ...
            if(Weight1!= Weight2){Weights = paste(Weight1, Weight2, sep = " and ")}
            <<<<<<< HEAD
            File_Name = paste("Nominal", Name_Group, Weights, Demographic_Category, sep = " - ")
            =======
              File_Name = paste("Responses", Name_Group, Weights, Demographic_Category, sep = " - ")
            >>>>>>> d48c4bb2bab115ed8f4e35058c2cd7830cebfd39
            Document_Title = gsub(" by Overall", "", paste(paste(Name_Group, ", Weighted by ", Weights, sep = ""), Demographic_Category, sep = " by "))
    ...
    Responses_to_Text = function(Responses, ColumnName, Codebook){
      ...
      if(Scale == "0 to 100"){
        ...
        Responses[as.numeric(Responses)>66] = Response_Positive
        Responses[as.numeric(Responses) < 67 & as.numeric(Responses) > 33] = Response_Neutral
        Responses[as.numeric(Responses)<33] = Response_Negative
      }
      ...
      if(Scale == "0 to 10"){
        ...
        # Replaces the numeric responses with text responses.
        Responses[as.numeric(Responses)>5] = Response_Positive
        Responses[as.numeric(Responses) == 5] = Response_Neutral
        Responses[as.numeric(Responses)<5] = Response_Negative
      }
      ...
      if(Scale == "1 to 5"){
        ...
        # Replaces the numeric responses with text responses.
        Responses[as.numeric(Responses)>3] = Response_Positive
        Responses[as.numeric(Responses) == 3] = Response_Neutral
        Responses[as.numeric(Responses)<3] = Response_Negative
      }
      ...
      if(Scale == "1 to 3"){
        ...
        # Replaces the numeric responses with text responses.
        Responses[as.numeric(Responses) == 1] = Response_Negative
        Responses[as.numeric(Responses) == 2] = Response_Neutral
        Responses[as.numeric(Responses) == 3] = Response_Positive
      }
      ...
      if(Scale == "0 to 1"){
        ...
        # Replaces the numeric responses with text responses.
        Responses[as.numeric(Responses) == 1] = Response_Positive
        Responses[as.numeric(Responses) == 0] = Response_Negative
      }
      
      # Condenses a difference scale
      if(Scale == "-10 to 10"){
        
        # Ensures there are no responses outside of the scale.
        if(suppressWarnings((max(na.omit(Responses)))>10)|suppressWarnings(min(na.omit(Responses)))<(0-10)){stop(paste("There is a response outside of the", Scale, "scale specified in the codebook for", ColumnName, sep = " "))}
        
        # Replaces the numeric responses with text responses.
        Responses[as.numeric(Responses) < 0] = Response_Negative
        Responses[as.numeric(Responses) == 0] = Response_Neutral
        Responses[as.numeric(Responses) > 0] = Response_Positive
      }
