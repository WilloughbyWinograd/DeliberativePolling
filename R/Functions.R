...
Crosstab.Demographics = function(Dataset, Template, Group1, Group2, Outputs, Export, Alpha, Only_Significant){
  ...
  repeat{
    ...
    Demographic_Category = Test_Chi_Squared(Demographic_Category, Frequencies_Group1[,1], Frequencies_Group2[,1])
...
Dataset.Organize = function(Codebook, Datasets){
  ...
  for(Dataset_Number in 1:length(Datasets)){
    ...
    for(Name in names(Codebook)){
      ...
      Dataset_Column_Number = which(names(Dataset) == Name) # Gets the column number in dataset corresponding to the column name from the codebook

      if(length(Dataset_Column_Number)>0){
