...
Responses_to_Text = function(Responses, ColumnName, Codebook){
  ...
  if(Scale == "0 to 100"){
    ...
    # Ensures there are no responses outside of the scale.
    if(suppressWarnings((max(na.omit(Responses)))>10)|suppressWarnings(min(na.omit(Responses)))<-10){stop(paste("There is a response outside of the", Scale, "scale specified in the codebook for", ColumnName, sep = " "))}
    ...
    Responses[Responses == 3] = Response_Positive
  } else if(Scale == "1 to 4"){
    
    # Ensures there are no responses outside of the scale.
    if(suppressWarnings((max(na.omit(Responses)))>4)|suppressWarnings(min(na.omit(Responses)))<1){stop(paste("There is a response outside of the", Scale, "scale specified in the codebook for", ColumnName, sep = " "))}
