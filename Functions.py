...
def ordinal_report( ####### UNFINISHED
    ...
    def stance_percentage(
        support_percent, opposition_percent, stance_negative, stance_positive
    ):
        if support_percent >= 0.67:
    ...
    if 2 <= len(legend) < 7:
        ...
        if not all_n_zero:
            ...
            for question in questions:
                ...
                lines = [
                    f'{tools.toTitleCase(subject)} were asked to respond to the statement, "{text}".'
                ]
                ...
                if API_Key != "None":
                    ...
                    lines = ask_chatgpt(
                        f"Rephrase the following to sound less structured and more human, but do not change the numbers or conclusions. Keep the numbers the same. Don't remove the word deliberation: {lines}"
                    )
                ...
                if data:
                    df = pd.DataFrame(
                        ...
                        columns=[
                            "Group",
                            f"Selected {stance_positive} Before Deliberation",
                            f"Selected {stance_positive} After Deliberation",
                        ],
                    )
                    if Demographic_Category != "Overall":
                    ...
                    for row, row_data in enumerate(
                        dataframe_to_rows(df, index=True, header=False)
                    ):
